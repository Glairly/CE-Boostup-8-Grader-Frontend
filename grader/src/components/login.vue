    ...
    <v-main>
      ...
      >
        <v-row align="center" justify="center">
          ...
          <v-card
            v-show="cardShow"
            class=" scale-in-center elevation-12 fab-trans"
            :width="$vuetify.breakpoint.mobile ? 700 : 1200"
            style="z-index:4"
          >
            <!-- header -->
            <v-flex class="d-flex   pa-0">
              <v-col cols="6" class="  pa-0">
                <v-sheet class="gradient t5-t2">
                  <v-img
                    :height="$vuetify.breakpoint.mobile ? 600 : 700"
              ...
              <v-col cols="6" class="  pa-0">
                ...
                >
                  ...
                  >
                    ...
                    <span class="text-h6 pa-2 font-weight-black"
                      >Login or "Try" to Register</span
                    >
                  </v-col>
                  <!-- error alert -->
                  <v-alert
                  ...
                  <v-col class="  px-3" cols="6" style="max-width:100%">
                    <v-form class="pa-5" ref="form" v-model="valid">
                      ...
                      <v-btn
                        rounded
                        class="ma-2 pa-6 glow-warning"
                        :class="scaleIn"
                        v-show="
                          loginValid &&
                            this.loginErrorMessage ==
                              'No data or User is not existed.'
                        "
                        @click.end="register()"
                        color="warning"
                        style="text-decoration-line:none;color:white"
                        >Register With This User ?</v-btn
                      >
                      <v-btn
                        rounded
                        class="ma-2 pa-6 glow-warning "
                    ...
                    >
                      ...
                      <v-card>
                        <v-row class="   justify-center  ">
                          <v-img
                            width="200px"
                            :src="require('@/assets/ig/' + ig.src + '.png')"
                          />
                        </v-row>
                      </v-card>
                    </v-menu>
          ...
          <v-dialog v-model="waitRegis" persistent width="500">
            <v-card color="indigo" dark>
              <v-card-text class="pa-5">
                ...
                >
                  <v-col>
                    <span class="mb-2 text-h6"
                      >Waiting For Registration...</span
                    >
      ...
      <template v-for="(c, index) in circleAround">
        <div
          :id="'bee' + index"
          :key="c"
          :class="c"
        ...
        >
          <v-img
            ...
            :src="require('@/assets/Bee_d.png')"
          ></v-img>
        </div>
      </template>
      <template v-for="(c, index) in linearBee">
        <div
          :id="'beeL' + index"
          :key="c"
          :class="c"
...
export default {
  ...
  methods: {
    register() {
      ...
      this.axios
        .post(this.$store.state.api + "/api/v1/register/", {
    ...
    login() {
      ...
      // Call login API
      this.axios
        .post(this.$store.state.api + "/api/v1/login/", {
          username: this.userFill,
          password: this.passFill,
        })
        .then((response) => {
  ...
  mounted() {
    this.moveBeeInit(["bee0", "bee1", "bee2", "beeL0", "beeL1"]);
    this.cardShow = true;
  },
  created() {},
};
